<template>
  <div>
        <div class="elevation-1">
          ...
          <v-img v-else-if="publication.main_picture!=null" aspect-ratio="1" cover class="white rounded-0" min-height="100%"
    ...
    <v-container>
      <v-row>
          ...
          <v-list color="transparent pa-0">
            <v-list-item class="grow pa-0">
              <v-list-item-avatar color="white mr-2">
                <v-img class="elevation-6" v-if="publication.user.profile_picture!=null" :src="publication.user.profile_picture.url" aspect-ratio="2" round>
                </v-img>
              </v-list-item-avatar>
              <v-list-item-content>
                <v-list-item-title class="font-weight-bold white--text">{{publication.user.username}}
                </v-list-item-title>
              </v-list-item-content>
              <v-list-item-action class="font-weight-bold mt-0 white--text">
        ...
        <v-col class="col-12 col-md-12"  v-if="publication.content!=null"> 
          <p class="font-weight-bold white--text text-left mb-0">
            {{publication.content}}
          </p>
        </v-col>
        <v-col class="col-12 col-md-12"  v-show="$auth.user.is_admin">
        ...
        <v-col class="col-12 col-md-12">
          <v-text-field placeholder="Tu comentario..." class="font-weight-bold rounded-lg" background-color="white" append-icon="ion-ios-send"
            @click:append="addComment()" filled hide-details v-model="comentario.contenido">
          </v-text-field>
        </v-col>
      </v-row>
      <v-list three-line dense color="transparent">
